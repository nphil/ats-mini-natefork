...
### Release assets (`firmware-auto-release` job in `build.yml`)

Every push to `main` auto-bumps `VER_APP` in `Common.h`, tags `vX.YY`, and publishes a GitHub release with:

| Asset | How to use |
|---|---|
| `ats-mini-vX.YY-ospi-ota.bin` | Web OTA upload via recovery UI |
| `ats-mini-vX.YY-ospi-flash.bin` | `esptool write_flash 0x0 …` — preserves LittleFS |
| `ats-mini-vX.YY-ospi-full.bin` | `esptool write_flash 0x0 …` — erases everything |
| `ats-mini-vX.YY-ospi-recovery.bin` | `esptool write_flash 0x10000 …` — factory partition, bootloader mode only |
| `ATSMini.ipa` (linked in notes) | Latest iOS build from most recent `ios-vX.Y.Z` release |
| `*.apk` (linked in notes) | Latest Android build from most recent `android-vX.Y.Z` release |

The release notes table is generated by `Generate release notes` step (Python) — it queries GitHub API for the latest `ios-v*` and `android-v*` releases and hyperlinks them in the notes. esptool commands appear in a `<details>` block.

### Pre-commit / lint rules

- **Trailing whitespace**: `pre-commit trailing-whitespace` fails on ANY line with trailing spaces, including blank lines. This applies to ALL files including `.yml` and `.ino`. No exceptions.
- Strip trailing whitespace with: `sed -i 's/[[:space:]]*$//' <files>`
- Mixed line endings: hooks enforce LF — don't commit CRLF files.

### Jobs in `build.yml`

- `build` — compiles recovery + main firmware, merges binaries, uploads artifact
- `release` — triggered only on `workflow_dispatch` or version tags; legacy manual path
- `firmware-auto-release` — runs on every push to `main`; canonical release job with `--make-latest=true`

### Removed / avoid re-adding

- **`latest-release` job** — removed. The repo ruleset blocks creation of the `latest` tag (HTTP 422). Versioned releases (`vX.YY`) are marked as "latest" via `--make-latest=true` on `gh release create`. GitHub's releases page shows the newest non-prerelease as latest automatically.
- **`Attach latest iOS IPA` step** — removed. The iOS IPA is now linked by URL in the generated release notes (no binary copy), which avoids re-downloading and re-uploading on each firmware build.
