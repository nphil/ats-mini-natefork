...
- **Recovery firmware — complete rewrite with Dracula theme and full encoder UI.**
  The recovery partition now has a fully interactive encoder-driven menu system styled
  with the [Dracula color palette](https://draculatheme.com/).

  **Boot trigger:** At power-on a KQ4TXO callsign splash screen appears with a 2-second
  countdown. Hold the encoder button during the splash to stay in recovery UI; releasing it
  auto-boots the main firmware. Three consecutive boot-loop crashes are detected and
  automatically keep the device in recovery (counter resets on successful main-app startup).

  **Main menu:** Boot Firmware / Erase Data (Yes/No confirmation popup) / Network / Reboot.

  **Network submenu:**
  - *WiFi Scan* — async background scan, scrollable results list; select a network then
    enter its password on a full QWERTY/numeric keyboard (encoder rotates columns,
    long-press moves rows, short-press selects key including DEL/SHIFT/123/OK).
    A live spinner shows connection progress; success returns to the Network menu.
  - *OTA Update* — fetches the last 25 GitHub `-ospi-ota.bin` releases over HTTPS,
    shows a scrollable list; select an entry to stream-download and flash with an
    on-screen progress bar, then auto-reboot.
  - *Web Server* — toggle on/off; always starts automatically on boot and tries
    `BEAST_ROUTER` then `IoT` before falling back to the `ATS-Recovery` AP.
    The web URL is shown on-screen.

  **Web server UI redesign:** Single-page dark Dracula app with:
  - Drag-and-drop (or click) `.bin` upload with XHR progress bar.
  - *GitHub Releases* tab showing the last 50 OTA releases; one-click flash with confirm dialog.
  - Erase Settings and Reboot buttons.

  Serial JSON OTA protocol for the Android app is unchanged.

## 2.56 (2026-06-21)

### Fixed

- **CI: `gh release create` flag corrected to `--latest`.** The previous `--make-latest=true`
  flag was rejected by the `gh` CLI version on GitHub Actions runners with
  `unknown flag: --make-latest`, causing every firmware-auto-release job to fail at the
  publish step. Changed to the correct boolean flag `--latest`.

## 2.55 (2026-06-21)

### Changed

- **CI: unified release assets.** Every push to `main` now publishes four firmware binaries
  (`-ospi-ota.bin`, `-ospi-flash.bin`, `-ospi-full.bin`, `-ospi-recovery.bin`) in a single
  `firmware-auto-release` job. Release notes are auto-generated by a Python step that queries
  the GitHub API for the latest iOS and Android releases and embeds download links and
  esptool commands in a collapsible `<details>` block.
  The deprecated `latest-release` job (which was blocked by a repo ruleset that prevented
  creation of the `latest` tag) has been removed.

## 2.54 (2026-06-21)

### Fixed

- **Battery text no longer overflows the battery icon on the VFO screen.** A call to
  `spr.setFreeFont(&Orbitron_Light_24)` in the VFO layout persisted into the next frame's
  sprite, causing `TFT_eSPI` to ignore the explicit font-1 argument in `drawString` and
  render the voltage/percentage text at 24 px (Orbitron) instead of 8 px (GLCD). The text
  stayed correct in menus because they reset the font themselves. Fixed by calling
  `spr.setTextFont(1)` at the start of `drawBattery()` to clear any active `gfxFont` pointer.
